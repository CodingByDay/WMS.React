...
export default function SerialQtyEntry(props) {
  ...
  useEffect(() => {
    ...
    document.getElementById("identEntry").value =
      props.data.ident.value + " " + props.data.name;
    ...
    TransactionService.getNextSSCC().then((response) => {
      document.getElementById("ssccEntry").value = DataAccess.getData(
        response,
        "SSCC",
        "StringValue",
      );
  ...
  function commitPositions(e) {
    ...
    var testObject = {
      ...
      Qty: qty,
      SSCC: sscc,
      LinkNo: props.data.no,
      LinkKey: props.data.key,
    };
