...
const Add = forwardRef((props, ref) => {
  ...
  function orderEdit(order, ident) {
    ...
    PopupService.getOrderDataFromIdentAndOrderNumber(order, ident).then(
      (response) => {
  ...
  function onChangeOrder(e) {
    if (e.value == "") {
      ...
      setOrderCurrent({
        label: e.key + " poz. " + e.no,
        value: e.key + " poz. " + e.no,
        key: e.key,
        no: e.no,
        deadline: e.deadline,
      });
    ...
    // Correct data gets to the service
    if (ident != "" && e.key != "") {
      PopupService.getOrderDataFromIdentAndOrderNumber(e.key, ident).then(
        (response) => {
          if (typeof response.Items !== "undefined") {
            var qty = DataAccess.getData(response, "OpenQty", "DoubleValue");
            var deadline = new Date(
  ...
  function updateOrders(identInternal) {
    ...
    TransactionService.getOrdersForIdent(identInternal, type).then(
      (response) => {
        var items = [];
        ...
        for (var i = 0; i < response.Items.length; i++) {
          ...
          var deadline = DataAccess.getData(
            item,
            "DeliveryDeadline",
            "DateTimeValue",
          );
  ...
  function CommitPositionSingular(old, data) {
    ...
    PopupService.commitPosition({
      ...
      HeadID: document.getElementById("transactionIdAdd").value,
  ...
  function CommitPosition(e) {
    if (editMode) {
      ...
      // Service callback
      TransactionService.getPositionsByHeadId(
        document.getElementById("transactionIdAdd").value,
      ).then((response) => {
        for (var i = 0; i < response.Items.length; i++) {
          ...
          if (ItemID == itemId) {
            var toUpdate = {
              ...
              DateInserted: DataAccess.getData(
                current,
                "DateInserted",
                "DateTimeValue",
              ),
              ...
              ClerkName: DataAccess.getData(
                current,
                "ClerkName",
                "StringValue",
              ),
              LocationName: DataAccess.getData(
                current,
                "LocationName",
                "StringValue",
              ),
              IssueLocationName: DataAccess.getData(
                current,
                "IssueLocationName",
                "StringValue",
              ),
              IdentName: DataAccess.getData(
                current,
                "IdentName",
                "StringValue",
              ),
      ...
      PopupService.getDocumentTypeStringBasedOnCode(
        ...
        if (response.includes("Prenos")) {
          ...
          response.includes("Naročilo") ||
          response.includes("Popravek zaloge prevzem")
        ) {
          PopupService.hasSerialNumberIdent(ident.value).then((response) => {
            data.serial = response.serial;
          ...
          response.includes("Proizvodnja") ||
          response.includes("DN")
        ) {
          // Delovni nalog
